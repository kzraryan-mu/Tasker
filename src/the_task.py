...
new_source_file = r'\\msits-srv-pdsql\ApplicationTeamNetworkShare\GradeRoster\GradeRosterEmpty.txt'
...
try:
    ...
    with pyodbc.connect('DSN={}'.format(dsn)) as connection:
        # read source file and upload it into a table
        new_source_df = pd.read_csv(source_file, sep='\t', dtype=str)
        insert_to_tmp_tbl_stmt = f"INSERT INTO {new_source_table} VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
        cursor = connection.cursor()
        cursor.fast_executemany = True
        cursor.executemany(insert_to_tmp_tbl_stmt, new_source_df.values.tolist())
        cursor.commit()
        cursor.close()
        # step0
